...
%static data
% ¾²Ö¹½×¶Î (2Ãë)
staticSamples = staticTime * fs;
staticPos = repmat([r, 0, 0], staticSamples, 1);
staticYaw = deg2rad(initialYaw);
staticOrient = repmat(quaternion([staticYaw, 0, 0], 'euler', 'ZYX', 'frame'), staticSamples, 1);
staticVel = zeros(staticSamples, 3);
staticAcc = zeros(staticSamples, 3);
staticAngVel = zeros(staticSamples, 3);
...
% ºÏ²¢¾²Ö¹½×¶ÎºÍÔË¶¯½×¶Î
trajData.Position = [staticPos; pos];
trajData.Orientation = [staticOrient; q];
trajData.Velocity = [staticVel; vel];
trajData.Acceleration = [staticAcc; acc];
trajData.AngularVelocity = [staticAngVel; av];
